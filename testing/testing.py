...
def pad_sequence(sequences: List[torch.Tensor], padding_side: str = 'right', padding_value: int = 0) -> torch.Tensor:
    ...
    for i, seq in enumerate(sequences):
        ...
        if padding_side == 'right':
            ...
            raise ValueError(f"padding_side must be either 'right' or 'left', got {padding_side}")
    return padded_seqs
...
def load_data(image_ids: List[str], targets: List[Dict], image_base_path: str) -> Dict[str, List]:
...
class LazySupervisedDataset(Dataset):
    def __init__(
        ...
        self.data: Dict[str, List] = {'image': [], 'label': []}

        all_image_ids, all_labels_dict = load_json_lines(data_args.label_file)
        if all_image_ids:
             self.data = load_data(all_image_ids, all_labels_dict, data_args.image_folder)
    ...
    def __getitem__(self, i: int) -> Dict[str, torch.Tensor]:
        ...
        prompt: str = (
            ...
            f"according to the following JSON schema: {fixed_schema_string}"
        )
